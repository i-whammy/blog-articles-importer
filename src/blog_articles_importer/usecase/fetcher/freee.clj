(ns blog-articles-importer.usecase.fetcher.freee
  (:require [blog-articles-importer.fetcher :as fetcher]
            [clojure.string :as s]
            [integrant.core :as ig]))

(def ^:private base-url "https://developers.freee.co.jp/rss")

(defn- ->article-id [url short-name]
  (let [slug (-> url
                 (s/replace #"https://developers.freee.co.jp/entry/" ""))]
  (str short-name slug)))
...
(defrecord FreeeFetcher
           [options]
  fetcher/Fetcher
  (execute [_ company] (fetch* company)))

(defmethod ig/init-key :blog-articles-importer.usecase.fetcher/freee [_ options]
  (map->FreeeFetcher options))