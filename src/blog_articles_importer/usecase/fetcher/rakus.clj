(ns blog-articles-importer.usecase.fetcher.rakus
  (:require [blog-articles-importer.fetcher :as fetcher]
            [clojure.string :as s]
            [integrant.core :as ig]))

(def ^:private base-url "https://tech-blog.rakus.co.jp/rss")

(defn- ->article-id [url short-name]
  (let [slug (-> url
                 (s/replace "https://tech-blog.rakus.co.jp/entry/" "")
                 (s/replace "/" ""))]
  (str short-name slug)))
...
(defrecord RakusFetcher
           [options]
  fetcher/Fetcher
  (execute [_ company] (fetch* company)))

(defmethod ig/init-key :blog-articles-importer.usecase.fetcher/rakus [_ options]
  (map->RakusFetcher options))