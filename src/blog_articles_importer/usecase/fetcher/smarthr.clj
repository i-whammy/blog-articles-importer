(ns blog-articles-importer.usecase.fetcher.smarthr
  (:require [blog-articles-importer.fetcher :as fetcher]
            [clojure.string :as s]
            [integrant.core :as ig]))

(def ^:private base-url "https://tech.smarthr.jp/rss")

(defn- ->article-id [url short-name]
  (let [slug (-> url
                 (s/replace "https://tech.smarthr.jp/entry/" "")
                 (s/replace "/" ""))]
  (str short-name slug)))
...
(defrecord SmarthrFetcher
           [options]
  fetcher/Fetcher
  (execute [_ company] (fetch* company)))

(defmethod ig/init-key :blog-articles-importer.usecase.fetcher/smarthr [_ options]
  (map->SmarthrFetcher options))