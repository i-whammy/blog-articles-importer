(ns blog-articles-importer.usecase.fetcher.zozo
  (:require [blog-articles-importer.fetcher :as fetcher]
            [clojure.string :as s]
            [integrant.core :as ig]))

(def ^:private base-url "https://techblog.zozo.com/rss")

(defn- ->article-id [url short-name]
  (let [slug (-> url
                 (s/replace "https://techblog.zozo.com/entry/" "")
                 (s/replace #"[/-]" ""))]
    (str short-name slug)))
...
(defrecord ZozoFetcher
           [options]
  fetcher/Fetcher
  (execute [_ company] (fetch* company)))

(defmethod ig/init-key :blog-articles-importer.usecase.fetcher/zozo [_ options]
  (map->ZozoFetcher options))