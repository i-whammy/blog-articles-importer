(ns blog-articles-importer.usecase.fetcher.yahoo
  (:require [blog-articles-importer.fetcher :as fetcher]
            [clojure.string :as s]
            [integrant.core :as ig]))

(def ^:private base-url "https://techblog.yahoo.co.jp/index.xml")
...
(defrecord YahooFetcher
           [options]
  fetcher/Fetcher
  (execute [_ company] (fetch* company)))

(defmethod ig/init-key :blog-articles-importer.usecase.fetcher/yahoo [_ options]
  (map->YahooFetcher options))