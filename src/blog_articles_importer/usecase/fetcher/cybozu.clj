(ns blog-articles-importer.usecase.fetcher.cybozu
  (:require [blog-articles-importer.fetcher :as fetcher]
            [clojure.string :as s]
            [integrant.core :as ig]))

(def ^:private base-url "https://blog.cybozu.io/rss")
...
(defrecord CybozuFetcher
           [options]
  fetcher/Fetcher
  (execute [_ company] (fetch* company)))

(defmethod ig/init-key :blog-articles-importer.usecase.fetcher/cybozu [_ options]
  (map->CybozuFetcher options))