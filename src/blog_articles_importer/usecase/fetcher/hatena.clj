(ns blog-articles-importer.usecase.fetcher.hatena
  (:require [blog-articles-importer.fetcher :as fetcher]
            [clojure.string :as s]
            [integrant.core :as ig]))

(def ^:private base-url "https://developer.hatenastaff.com/rss")
...
(defrecord HatenaFetcher
           [options]
  fetcher/Fetcher
  (execute [_ company] (fetch* company)))

(defmethod ig/init-key :blog-articles-importer.usecase.fetcher/hatena [_ options]
  (map->HatenaFetcher options))