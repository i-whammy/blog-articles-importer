(ns blog-articles-importer.usecase.fetcher.legalforce
  (:require [blog-articles-importer.fetcher :as fetcher]
            [clojure.string :as s]
            [integrant.core :as ig]))

(def ^:private base-url "https://tech.legalforce.co.jp/rss")
...
(defrecord LegalForceFetcher
           [options]
  fetcher/Fetcher
  (execute [_ company] (fetch* company)))

(defmethod ig/init-key :blog-articles-importer.usecase.fetcher/legalforce [_ options]
  (map->LegalForceFetcher options))