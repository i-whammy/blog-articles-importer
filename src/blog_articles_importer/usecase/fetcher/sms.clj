(ns blog-articles-importer.usecase.fetcher.sms
  (:require [blog-articles-importer.fetcher :as fetcher]
            [clojure.string :as s]
            [integrant.core :as ig]))

(def ^:private base-url "https://tech.bm-sms.co.jp/rss")
...
(defrecord SmsFetcher
           [options]
  fetcher/Fetcher
  (execute [_ company] (fetch* company)))

(defmethod ig/init-key :blog-articles-importer.usecase.fetcher/sms [_ options]
  (map->SmsFetcher options))