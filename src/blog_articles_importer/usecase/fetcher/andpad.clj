(ns blog-articles-importer.usecase.fetcher.andpad
  (:require [blog-articles-importer.fetcher :as fetcher]
            [clojure.string :as s]
            [integrant.core :as ig]))

(def ^:private base-url "https://tech.andpad.co.jp/rss")
...
(defrecord AndpadFetcher
           [options]
  fetcher/Fetcher
  (execute [_ company] (fetch* company)))

(defmethod ig/init-key :blog-articles-importer.usecase.fetcher/andpad [_ options]
  (map->AndpadFetcher options))